...
export default class GoogleDriveAPI {
  ...
  static _initResumableSession(name, type) {
    return call('post', 'files', { 'Content-Type': 'application/json; charset=UTF-8' }, { 'uploadType': 'resumable' }, // query
  ...
  static importFileResumable(file, name, type) {
    return this._initResumableSession(name, type).then(response => {
